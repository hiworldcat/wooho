...
4. Inspect each episode:
   ...
   - low-dimensional state/action checks;
   - geometry-gated Vision-State-Vision multi-camera verification;
   - regular vision-state temporal and motion consistency checks.
...
- `actions` has the same 20-D layout and is used as fallback if `state` is not
  available
...
Only enabled pairs are passed to the visual verifier. The verifier checks
motion correlation against the reference distribution, but the result is treated
as a weak signal and is emitted under `vision_state_vision` (`1.1.2.C`), not
under direct `vision_vision` (`1.1.2.B`).
